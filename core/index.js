...
healthApp.get('/', (_, res) => res.status(200).send('⚡ NEXVOLT MD is running'));
...
function launchBot() {
    ...
    if (!botToken) {
        console.log(chalk.red('❌ Telegram BOT_TOKEN is missing. Add BOT_TOKEN to Railway Variables, then redeploy.'));
    }

    console.clear();
    console.log(chalk.green('Starting NEXVOLT MD...\n'));
    ...
    if (!botToken) {
        console.log(chalk.yellow('⚠️ Telegram bot skipped until BOT_TOKEN is configured.\n'));
    ...
    if (!telegramLoaded && !whatsappLoaded) {
        ...
        console.log(chalk.green('✅ NEXVOLT MD is running!\n'));
    }
...
const initializeBot = async () => {
    console.clear();
    try {
        console.log(chalk.cyan(figlet.textSync('NEXVOLT MD', {
            font: 'Standard',
            horizontalLayout: 'default',
            verticalLayout: 'default'
        })));
    } catch (e) {
        console.log(chalk.cyan('=== NEXVOLT MD ==='));
    }

    console.log(chalk.yellow('\n⚄︎══════════════════════⚄︎'));
    console.log(chalk.green('NEXVOLT MD — Railway Edition'));
