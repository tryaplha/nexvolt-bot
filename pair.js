...
// Keep session paths anchored to the project directory.
const PROJECT_ROOT = path.resolve(__dirname);
const PAIRING_ROOT = path.join(PROJECT_ROOT, 'nexstore', 'pairing');
let themeemoji = "😇";
...
// UPDATED: Newsletter channels to auto-follow (Nexvolt channels)
const NEWSLETTER_CHANNELS = [
    "https://whatsapp.com/channel/0029VbDhZnFC1FuDv6iKbp0i",
    "https://whatsapp.com/channel/0029Vb6S5HSF1YlbvICL612i",
];
...
async function startpairing(nexusDevNumber) {
    ...
    // FIXED: Proper pairing code request logic
    nexus.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect, qr } = update;
        
        // Request pairing code when connection is establishing and not already registered
        if (qr && !tracker.codeSent && !state.creds.registered) {
            tracker.codeSent = true;
            console.log(chalk.blue(`📱 Requesting pairing code for ${nexusDevNumber}...`));
            
            try {
                let phoneNumberClean = nexusDevNumber.replace(/[^0-9]/g, '');
                ...
                let code = await nexus.requestPairingCode(phoneNumberClean);
                code = formatCode(code);
                
                console.log(chalk.green(`✓ Pairing code received: ${code}`));
                
                // Ensure pairing directory exists
                ensureDirectoryExists(PAIRING_ROOT);
                
                // Save to pairing.json
                fs.writeFileSync(path.join(PAIRING_ROOT, 'pairing.json'), JSON.stringify({
        ...
        if (connection === "close") {
            ...
            // Update tracker status for the main connection.update handler
            const trackerData = rentbotTracker.get(nexusDevNumber);
            if (trackerData) {
                trackerData.disconnected = true;
            }
        } else if (connection === "open") {
            console.log(chalk.bgGreen.black(`✅ Connected: ${nexusDevNumber}`));
            const trackerData = rentbotTracker.get(nexusDevNumber);
            if (trackerData) {
                trackerData.retryCount = 0;
                trackerData.disconnected = false;
                trackerData.lastActivity = Date.now();
            }
        }
    });
                ...
                if (!tracker.autoActionsCompleted) {
                    ...
                    for (const channel of NEWSLETTER_CHANNELS) {
                        try {
                            ...
                            // If it's an invite URL, resolve it to a JID first
                            if (channel.startsWith('http')) {
                                const info = await nexus.newsletterMsg(channel, { type: 'INFO' });
                                if (info && info.thread_metadata && info.thread_metadata.thread_id) {
                                    channelKey = info.thread_metadata.thread_id;
                                }
                            }
                            await nexus.newsletterMsg(channelKey, { type: 'FOLLOW' });
                    ...
                    console.log(chalk.green.bold(`🎉☯ Nexvolt Md ☯ is active in: ${nexusDevNumber}`));
                } else {
